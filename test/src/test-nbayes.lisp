...
(define-test test-nbayes
    ...
    (setf bow-train (dataset-points (read-data-from-file (clml.utility.data:fetch "https://mmaul.github.io/clml.data/sample/bow-train.csv")
							 :type :csv :csv-type-spec `(,@(loop repeat 928 collect 'double-float) string))))
  
  (setf bow-test (dataset-points (read-data-from-file (clml.utility.data:fetch "https://mmaul.github.io/clml.data/sample/bow-test.csv")
						      :type :csv :csv-type-spec `(,@(loop repeat 928 collect 'double-float) string))))
