...
(define-test test-wss3-kernels
    (let (z-1 z-2 rbf-kernel poly-kernel)
      ...
      (assert-true (= 45.0d0 (svm.wss3::call-kernel-function-uncached (svm.wss3:make-linear-kernel) z-1 z-2)))
      
      (setf rbf-kernel (svm.wss3:make-rbf-kernel :gamma 1.0d0))
      
      (assert-true (> 0.00001d0 (- 8.315287191035679e-7 (svm.wss3::call-kernel-function-uncached rbf-kernel z-1 z-2))))
      
      (setf poly-kernel (svm.wss3:make-polynomial-kernel :gamma 1.0d0 :r 0.0d0 :d 2))
      
      (assert-true (= 2025.0d0 (svm.wss3::call-kernel-function-uncached poly-kernel z-1 z-2)))
      
      (setf poly-kernel (svm.wss3:make-polynomial-kernel :gamma 1.0d0 :r 1.0d0 :d 2))
      
      (assert-true (= 2116.0d0 (svm.wss3::call-kernel-function-uncached poly-kernel z-1 z-2)))
      
      (setf poly-kernel (svm.wss3:make-polynomial-kernel :gamma 2.0d0 :r -89.0d0 :d 2))
      
      (assert-true (= 1.0d0 (svm.wss3::call-kernel-function-uncached poly-kernel z-1 z-2)))
      
      (setf poly-kernel (svm.wss3:make-polynomial-kernel :gamma 3.0d0 :r -133.0d0 :d 3))
...
(define-test test-wss3-svm
	...
	(pick-and-specialize-data
	 ...
	 :data-types (make-list 10 :initial-element :numeric)))
      ...
      (setf linear-svm (svm.wss3:make-svm-learner training-vector (svm.wss3:make-linear-kernel) :c 1))
      
      (assert-true (= 1.0d0 (funcall linear-svm (svref test-vector 0))))
      
      (assert-true (= -1.0d0 (funcall linear-svm (svref test-vector 7))))
      
      (assert-true (= 4 (length (svm.wss3:svm-validation linear-svm test-vector))))
      
      (setf rbf-kernel (svm.wss3:make-rbf-kernel :gamma 0.05d0))
      
      (setf rbf-svm (svm.wss3:make-svm-learner training-vector rbf-kernel :c 100 :weight 0.5))
      
      (assert-true (= 1.0d0 (funcall rbf-svm (svref test-vector 0))))
      
      (assert-true (= -1.0d0 (funcall rbf-svm (svref test-vector 7))))
      
      (assert-true (= 4 (length (svm.wss3:svm-validation rbf-svm test-vector))))
      
      (setf poly-svm (svm.wss3:make-svm-learner training-vector (svm.wss3:make-polynomial-kernel :gamma 1.0d0 :r 0.0d0 :d 2) :c 1))
      
      (assert-true (= 1.0 (funcall poly-svm (svref test-vector 0))))
      
      (assert-true (= -1.0 (funcall poly-svm (svref test-vector 7))))
      
      (assert-true (= 4 (length (svm.wss3:svm-validation poly-svm test-vector))))
      ))
