...
(define-test test-pwss3-kernels
  (let (poly-kernel rbf-kernel z-1 z-2)
    ...
    (assert-true (setf rbf-kernel (clml.svm.pwss3::make-rbf-kernel :gamma 1.0d0)))
    (assert-true (> 0.00001d0 (- 8.315287191035679e-7 (clml.svm.pwss3::call-kernel-function-uncached rbf-kernel z-1 z-2))))
    (assert-true (setf poly-kernel (clml.svm.pwss3::make-polynomial-kernel :gamma 1.0d0 :r 0.0d0 :d 2)))
    (assert-true (= 2025.0d0 (clml.svm.pwss3::call-kernel-function-uncached poly-kernel z-1 z-2)))
    (assert-true (setf poly-kernel (clml.svm.pwss3::make-polynomial-kernel :gamma 1.0d0 :r 1.0d0 :d 2)))
    (assert-true (= 2116.0d0 (clml.svm.pwss3::call-kernel-function-uncached poly-kernel z-1 z-2)))
    (assert-true (setf poly-kernel (clml.svm.pwss3::make-polynomial-kernel :gamma 2.0d0 :r -89.0d0 :d 2)))
    (assert-true (= 1.0d0 (clml.svm.pwss3::call-kernel-function-uncached poly-kernel z-1 z-2)))
    (assert-true (setf poly-kernel (clml.svm.pwss3::make-polynomial-kernel :gamma 3.0d0 :r -133.0d0 :d 3)))
...
(define-test test-pwss3-svm
  (let (accuracy linear-svm poly-svm rbf rbf-kernel rbf-svm results
    ...
    (assert-true (setf linear-svm (clml.svm.pwss3::make-svm-learner training-vector (clml.svm.pwss3:make-linear-kernel) :c 1))) ;Not sure whether is c or weight
    ...
    (assert-true (setf rbf-kernel (clml.svm.pwss3::make-rbf-kernel :gamma 0.05d0)))
    (assert-true (setf rbf-svm (clml.svm.pwss3:make-svm-learner training-vector rbf-kernel :c 100)))
    ...
    (assert-true (setf poly-svm (clml.svm.pwss3::make-svm-learner training-vector (clml.svm.pwss3:make-polynomial-kernel :gamma 1.0d0 :r 0.0d0 :d 2) :c 1)))
    ...
    (assert-true (setf train-data (pick-and-specialize-data (read-data-from-file (clml.utility.data:fetch "https://mmaul.github.io/clml.data/sample/svm-benchmark-train.csv") :type :csv :csv-type-spec (make-list 24 :initial-element 'double-float)
                                                                                 :external-format :utf-8) :data-types (make-list 24 :initial-element :numeric))))
    (assert-true (setf test-data (pick-and-specialize-data (read-data-from-file (clml.utility.data:fetch "https://mmaul.github.io/clml.data/sample/svm-benchmark-test.csv") :type :csv :csv-type-spec (make-list 24 :initial-element 'double-float)
                                                                                :external-format :utf-8) :data-types (make-list 24 :initial-element :numeric))))
    (assert-true (setf training-vector (dataset-points train-data)))
    (assert-true (setf test-vector (dataset-points test-data)))
    (assert-true (setf rbf (clml.svm.pwss3:make-rbf-kernel :gamma 0.5d0)))
    (assert-true (setf svm (clml.svm.pwss3:make-svm-learner training-vector rbf :c 10)))
    (assert-true (multiple-value-setq (results accuracy) (clml.svm.pwss3:svm-validation svm test-vector)))
